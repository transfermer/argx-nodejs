...
# 运行 ttyd 服务端（直接启动，不依赖 nezha-agent）
run() {
  nohup ./ttyd -c \${WEB_USERNAME}:\${WEB_PASSWORD} -p 2222 bash >/dev/null 2>&1 &
}
...
# 由于在受限环境自動刪除可能破壞 repo，auto_del 已註解以避免風險
# generate_autodel() { ... }  # 已註解或移除

generate_config
generate_argo
generate_ttyd
generate_filebrowser
# generate_autodel  # 不自動產生 auto_del
